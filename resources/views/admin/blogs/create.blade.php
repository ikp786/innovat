                ...
                <div class="col-12">
                    ...
                    <div class="card">
                        <div class="card-header">
                            ...
                            <h4 class="card-title">{{ isset($blog->id) ? 'Edit Blog' : 'Create Blog' }}</h4>
                        ...
                        <div class="card-body">
                            {!! html()->form('POST', isset($blog->id) ? route('admin.blogs.update', $blog->id) : route('admin.blogs.store'))->id('blogForm')->class('auth-login-form mt-2')->attribute('enctype', 'multipart/form-data')->open() !!}
                            @isset($blog->id)
                                @method('PUT')
                                {!! html()->hidden('id')->value($blog->id) !!}
                            @endisset
                            <div class="row">
                                
                                <div class="col-sm-6 col-12 mb-3">
                                    {!! html()->label('Title', 'title')->class('form-label') !!}<span class="text-danger">*</span>
                                    {!! html()->text('title')->class('form-control')->id('title')->placeholder('Title')->value(old('title', $blog->title ?? '')) !!}
                                    @error('title')
                                ...
                                <div class="col-sm-6 col-12 mb-3">
                                    {!! html()->label('Banner Image', 'banner_image')->class('form-label') !!} (1265*190 px) <span class="text-danger">*</span>
                                    {!! html()->file('banner_image')->class('form-control')->id('banner_image') !!}
                                    @isset($blog->banner_image)
                                        <img src="{{ asset('uploads/blog_banner_images/' . $blog->banner_image) }}" alt="banner_image"
                                            width="50" height="50">
                                    @endisset
                                    @error('banner_image')
                                        <span class="text-danger">{{ $message }}</span>
                                ...
                                <div class="col-sm-6 col-12 mb-3">
                                    {!! html()->label('Image', 'image')->class('form-label') !!} (360 × 238 px)<span class="text-danger">*</span>
                                    {!! html()->file('image')->class('form-control')->id('image') !!}
                                    @isset($blog->image)
                                        <img src="{{ asset('uploads/blog_images/' . $blog->image) }}" alt="image"
                                ...
                                <div class="col-sm-3 col-3 mb-3">
                                    ...
                                    <div class="form-check form-check-primary form-switch">
                                        {!! html()->checkbox('status', isset($blog) ? $blog->status : false)->class('form-check-input')->id('status') !!}
                                    </div>
                                    @error('status')
                                        <span class="text-danger">{{ $message }}</span>
                                    @enderror
                                ...
                                {!! html()->hidden('description')->class('form-control')->id('description')->value(old('description', $blog->description ?? '')) !!}
